"""
QA tester tools: read/write files and list directory within a workspace.
Used by the QA Tester Agent to inspect code, generate tests, and update configs.
"""
import os
...
class ReadFileInput(BaseModel):
    ...
    path: str = Field(
        description=(
            ...
            "(e.g. 'src/Login.tsx', 'package.json', 'tests/login.spec.ts')."
        )
    )
...
class ReadFileTool(BaseTool):
    ...
    name: str = "read_file"
    description: str = (
        "Read the contents of a file. Use this to inspect code, existing tests, "
        "or configs before designing or updating tests. Path is relative to workspace root."
    )
...
class WriteFileTool(BaseTool):
    ...
    description: str = (
        ...
        "test files, fixtures, or configs. Path is relative to workspace root. "
...
class ListDirectoryTool(BaseTool):
    ...
    description: str = (
        ...
        "locate src and test folders, or discover existing test suites. "
...
def get_qa_tools(workspace_root: Optional[str] = None) -> list:
